...
本命令的执行过程（阶段选择依据、搜索与调研、方案取舍）只被压缩进最终计划文档——为中断诊断与决策追溯，边执行边追加一份**纯旁路**过程日志。

### 路径与命名

`docs/PRPs/logs/{plan-name}.exec.log`。`{plan-name}` = 所选阶段名称的 kebab-case（与阶段 6 plan 文件名同源），阶段 0 选定阶段后即可推导；free-form 输入时在阶段 1 明确 feature 后推导，此前不落盘，推导出后创建并把已积累的过程并入首条。首次写入时 `mkdir -p docs/PRPs/logs` 并创建头部；文件已存在（重跑/续跑）则直接追加，不重建头部。
...
```markdown
# Exec: {plan-name}
- PRD: <路径或 N/A> | 创建: prp-plan <yyyymmdd-HHMM>
...
```markdown
## [prp-plan <yyyymmdd-HHMM>] 阶段 N — <主题>
- 动作: <做了什么>
- 决策/依据: <选择与理由；被否决项>
- 偏差: <与预期的差异>
- 结果: <产物/下一步>
...
| 时机 | 条目内容 |
|---|---|
| 阶段 0 选定阶段后（创建 exec.log） | 输入类型、PRD 路径、选定阶段（标识/名称）、依赖链选择依据 |
| 阶段 1 复杂度评估后 | 复杂度等级及指标依据；**歧义门触发时 STOP 前追加**问题原文与缺失信息 |
| 阶段 2 每类搜索后（或阶段末批量） | 搜索类别 + query + 命中文件数 + 采纳的关键发现（trace 路径、模式文件） |
| 阶段 3 后 | 研究问题清单 + 采纳的外部结论及来源 |
| 阶段 4/5 后 | 设计/架构决策：所选方案、被否决备选及理由 |
| 阶段 6 输出后 | plan 落盘路径、PRD 状态更新结果（指针） |
...
- **只写不读**：计划生成不读取 exec.log——它是给人读的旁路记录，不是事实源。
- **不记边界**：完整搜索输出与代码 diff 不落盘（终端与 git 已承载）；最终结论不重述（plan 文档承载，只记指针）。
