...
2. **先行测试（条件执行）** — 若计划「测试策略」中与本任务关联的测试（按「单元测试」表「关联任务」列对应；无该列的旧计划按测试名称/场景对应）标注了 **先行验证**，先编写该测试并运行，确认其**失败**（记录失败输出作为 red 证明），然后才进入实现。实现完成后同一测试必须通过。未标注的任务跳过此步。该判定仅适用于「单元测试」表的用例；集成测试表无先行验证列，一律在 Level 4 实现后编写。
