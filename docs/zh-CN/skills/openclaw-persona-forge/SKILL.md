...
**Agent 执行**：
1. 将本 SKILL.md 文件所在的目录路径确定为 `SKILL_DIR`
...
| 用户说 | 执行模式 |
|--------|---------|
| "帮我设计龙虾灵魂" / "我想给龙虾定个性格" | → **引导模式**（步骤 1） |
| "抽卡" / "随机" / "来一发" / "盲盒" / "gacha" | → **抽卡模式**（步骤 1-B） |
| "帮我优化这个灵魂" / 附带已有 SOUL.md | → **打磨模式**（跳到步骤 4） |
