...
description: PyTorch 深度学习模式和最佳实践，用于构建稳健、高效和可复现的训练管道、模型架构和数据加载。
metadata:
  origin: ECC
---
...
构建健壮、高效且可复现的深度学习应用程序的惯用 PyTorch 模式和最佳实践。
...
编写代码时，务必确保代码既能在 CPU 上运行，也能在 GPU 上运行，不要硬编码设备类型。
...
__请牢记__: PyTorch 代码应该与设备无关、可复现且注重内存使用。如有疑问，请使用 `torch.profiler` 进行性能分析，并使用 `torch.cuda.memory_summary()` 检查 GPU 内存。
