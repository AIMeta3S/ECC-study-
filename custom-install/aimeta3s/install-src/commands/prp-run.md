...
调度层信息（断点推导、dispatch 摘要、核验证据、STOP 原因）不会出现在任何命令产物中——为追溯与问题查找，每次运行落盘一份**纯旁路**日志。命令内部过程由各被调度命令按其文档另行写入 `docs/PRPs/logs/{plan-name}.exec.log`（过程日志，同样的只写不读/best-effort 精神）——run log 记调度层、exec.log 记命令层，互不重叠；本命令的核验清单与派发协议产物列均**不**纳入 exec.log。
