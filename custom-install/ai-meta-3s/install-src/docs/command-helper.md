...
本指南面向 `commands/` 下的 **39 个命令**。这些命令数量多、族系复杂，单看每个命令文件难以判断"我这个任务到底该用哪条"。本文档把视角从「按命令查」翻转为「按场景用」：
...
按族归类的 39 个命令，每个一句话用途；带 commit 前缀的标注前缀。
...
*本指南基于 `commands/` 下 39 个命令文件的用途、执行阶段与命令间显式串联关系整理。命令的内部步骤请参阅各命令文件本身。*
