...
不是理论上的。不是在沙盒中。我当时正在测试一个社区频道中有人分享的 ClawdHub 技能——一个受欢迎的、被其他用户推荐的技能。表面上看起来很干净。一个合理的 task 定义，清晰的说明，格式良好的 Markdown。
...
> **\[图片：终端截图显示一个 ClawdHub 技能文件，其中包含一个高亮显示的隐藏指令——顶部是可见的 task 定义，下方显示被注入的系统指令。已涂改但显示了模式。]**
...
* “如果我的代理点击了钓鱼链接会发生什么？”（它会以执行合法 tasks 时相同的权限，遵循被注入的指令。）
...
CrowdStrike 称 OpenClaw 是一个“能够接受对手指令的强大 AI 后门代理”，并警告它制造了一种“独特危险的情况”，即提示注入“从内容操纵 issue 转变为全面入侵的推动者”。
...
OWASP 引入了一个称为 **最小自主权** 的原则：只授予代理执行安全、有界 tasks 所需的最小自主权。OpenClaw 的架构恰恰相反——它默认连接到尽可能多的通道和工具，从而最大化自主权，而沙盒化则是一个事后才考虑的附加选项。
...
**6. 事件响应。** 当（不是如果）发生安全 issue 时，提供商有一个处理流程：检测、遏制、通知、补救。而不是“去 Discord 查看更新”。
...
1. **审核您安装的每一个 ClawdHub 技能。** 阅读完整的源代码，而不仅仅是可见的描述。查找 task 定义下方的隐藏指令。如果您无法阅读源代码并理解其作用，请将其移除。
...
诚实地问问自己：你是需要多频道编排，还是需要一个能执行 tasks 的 AI 智能体？这是两件不同的事情。智能体功能可以通过 Claude Code、Cursor、Codex 和其他工具链获得——而无需承担多频道攻击面。
