...
| 留空 · 扫描 | 依次探测 `.claude/reviews/` 与遗留 `.claude/PRPs/reviews/`，收集所有 `local-*-review.md`，按文件名内嵌时间戳 `yyyymmdd-HHMM` 跨目录取最新（不依赖 mtime） |
...
- [ ] 重跑 /code-review --prp <plan-name>（或原档位）复审
...
后续步骤：
  - 重跑 /code-review --prp <plan-name>（或原档位）复审 —— 决策 PASS 前不要 /prp-commit
...
| 遗留目录 `.claude/PRPs/reviews/` | 与 `.claude/reviews/` 同等探测，fix-report 落在源报告同目录 |
