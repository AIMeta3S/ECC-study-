...
try {
  ...
  // 安装到 ~/.claude 时无同级 package.json，降级为 'unknown'；server 版本比对仍自洽
  VERSION = 'unknown';
}
...
function usage() {
  return [
    ...
    '',
    'Usage:',
    `  ${SELF_CMD}                      Show server status and sessions`,
    `  ${SELF_CMD} open <file>          Open (or resume) a review session`,
    `  ${SELF_CMD} await <file>         Block until the human sends feedback`,
    `  ${SELF_CMD} pending              Show feedback queued for no listener`,
    `  ${SELF_CMD} typing <file>        Show a thinking/typing indicator in chat`,
    `  ${SELF_CMD} end <file>           End a session as the agent`,
    `  ${SELF_CMD} stop                 Shut down the canvas server`,
    `  ${SELF_CMD} server               Run the server in the foreground`,
    '',
    'Options:',
