...
使用 `style` prop 添加额外的 CSS（使用 `width` 和 `height` prop 设置尺寸）：
...
使用 `@remotion/gif` 中的 `getGifDurationInSeconds()` 获取 GIF 的时长。
...
这对于设置 composition 时长以匹配 GIF 很有用：
...
如果 `<AnimatedImage>` 不起作用（仅在 Chrome 和 Firefox 中支持），可以改用 `@remotion/gif` 中的 `<Gif>`。
...
`<Gif>` 组件具有与 `<AnimatedImage>` 相同的 props，但仅支持 GIF 文件。
